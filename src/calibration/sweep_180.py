"""
SCARA Calibration — 180° Rigid Arm Sweep
==========================================
Moves both motors synchronously to validate the full 180° range.
Used for verifying joint alignment and adjusting the gamma offset.
...
class ScaraSyncApp(tk.Tk):
    """Calibration app for synchronized rigid-arm sweep."""
    ...
    def _setup_ui(self) -> None:
        ...
        self.lbl_gamma = ttk.Label(
            gamma_frame, text=f"Gamma Actual: {self.gamma:.2f}°",
            font=("Arial", 10, "bold"),
        )
        self.lbl_gamma.pack()
        ...
        for val in [-5, -1, -0.1, 0.1, 1, 5]:
            ttk.Button(
                btn_box, text=f"{val:+}°", width=5,
                command=lambda v=val: self.update_gamma(v),
        ...
        self.slider = ttk.Scale(
            sync_frame, from_=0, to=180, variable=self.arm_angle_var,
            orient=tk.HORIZONTAL, command=self.on_slider_move,
        )
        self.slider.pack(fill=tk.X, pady=10)
        self.lbl_pos = ttk.Label(
            sync_frame, text="Brazo en: 90.00° (Centro)",
            font=("Arial", 12, "bold"), foreground="#007AFF",
        )
        self.lbl_pos.pack()
    ...
    def update_gamma(self, delta: float) -> None:
        self.gamma += delta
        self.lbl_gamma.config(text=f"Gamma Actual: {self.gamma:.2f}°")
        self.send_command()

    def set_arm_pos(self, val: float) -> None:
        self.arm_angle_var.set(val)
        self.send_command()

    def on_slider_move(self, _event) -> None:
        self.send_command()
